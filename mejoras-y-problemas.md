...
* **Calidad y CI:** linters (PSScriptAnalyzer, shellcheck, `bash -n`), suite bash de la variante Linux (53 pruebas), suites de lógica de `setup.ps1` (24) y del módulo de lanzadores (36) ejecutables en CI, verificación del quoting del lanzador C contra reglas MSVCRT en cada push y cadena completa semanal/manual en `windows-latest` (setup → smoke → empaquetado → roundtrip offline).
