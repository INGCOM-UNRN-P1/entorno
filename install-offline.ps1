# install-offline.ps1 - Asiste la instalación del paquete offline (portable-env-offline.zip).
# Extrae el ZIP en el destino elegido, valida la estructura y deja el entorno listo para
# ejecutar launch.bat. No requiere permisos de administrador ni conexión a internet.
#
# Uso:
...
if (-not (Test-Path $RutaZip)) {
    Write-Error "No se encontró el paquete '$RutaZip'. Descargalo o indicá su ruta con -RutaZip."
    exit 1
}
...
Write-Host "=== Instalación Offline del Entorno Portable ===" -ForegroundColor Cyan
Write-Host "Paquete : $RutaZip ($([math]::Round((Get-Item $RutaZip).Length / 1GB, 2)) GB)"
Write-Host "Destino : $Destino"
Write-Host ""

# Extracción acelerada: tar.exe (bsdtar incluido desde Windows 10 1803) con fallback nativo
$tarExe = Join-Path $env:SystemRoot "System32\tar.exe"
if (Test-Path $tarExe) {
    Write-Host "[1/3] Extrayendo con tar.exe (rápido)..." -ForegroundColor Cyan
    & $tarExe -xf "$RutaZip" -C "$Destino"
    if ($LASTEXITCODE -ne 0) {
        Write-Warning "tar.exe reportó código $LASTEXITCODE; reintentando con Expand-Archive..."
...
# Validación estructural mínima del paquete
Write-Host "[2/3] Validando estructura extraída..." -ForegroundColor Cyan
$launchBat = Get-ChildItem -Path $Destino -Filter "launch.bat" -Recurse | Select-Object -First 1
if (-not $launchBat) {
    Write-Error "El paquete no contiene launch.bat. ¿Es un portable-env-offline.zip válido?"
    exit 1
}
$rootExtraido = Split-Path $launchBat.FullName -Parent

# Advertencia clásica de rutas conflictivas para compiladores
$hasConflict = ($rootExtraido -match ' ') -or ($rootExtraido -match '[^\u0000-\u007F]') -or ($rootExtraido -match '(?i)onedrive|dropbox')
if ($hasConflict) {
    Write-Warning "La ruta '$rootExtraido' contiene espacios, caracteres no ASCII o está sincronizada; puede romper las herramientas de compilación."
    Write-Warning "Se recomienda mover la carpeta a una ruta simple (ej: C:\dev\entorno)."
}

Write-Host "[3/3] Listo." -ForegroundColor Cyan
Write-Host ""
Write-Host "=== INSTALACIÓN OFFLINE COMPLETADA ===" -ForegroundColor Green
...
Write-Host "  1. Abrí el entorno con launch.bat (desde esa carpeta)."
Write-Host "  2. Configurá tu identidad: configure-git.sh"
Write-Host "  3. Verificá la salud: smoke.sh"
