...
*   **Cero Huella en Linux:** La variante `linux/` no debe modificar nada del sistema host ni requerir permisos de administrador: ninguna instalación automática, ningún `sudo`. Toda escritura se limita al repositorio (`home/`, `local/`) y a la sesión de terminal activa; las dependencias del sistema solo se diagnostican y sugieren, nunca se instalan.
